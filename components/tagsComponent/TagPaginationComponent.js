...
const TagPaginationComponent = ({currentPage,totalItems,totalPages,path,sourceType,pageSize}) => {
    ...
    return (
        <div className="flex items-center justify-between border-t border-gray-200 bg-neutral-800 px-4 py-3 sm:px-6">
            <div className="flex flex-1 justify-between sm:hidden">
                <Link
                    href={{pathname:path,query:{page:currentPage-1,sourceType:sourceType}}}
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                ...
                <div>
                    <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="TagPaginationComponent">
                        <Link
                            href={currentPage===1?``:{pathname:path,query:{page:currentPage-1,sourceType:sourceType}}}
                        ...
                        {paginationRange.map((pageNumber)=>(
                            <Link key={parseInt(pageNumber)}
                                  href={{pathname:path,query:{page:parseInt(pageNumber),sourceType:sourceType}}}
                                  aria-current="page"
                        ...
                        <Link
                            href={currentPage===totalPages?`#`:{pathname:path,query:{page:currentPage+1,sourceType:sourceType}}}
...
export default TagPaginationComponent;
